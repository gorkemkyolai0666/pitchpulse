...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    const token = this.jwtService.sign({ userId: user.id, studioId: studio.id });
    return {
      accessToken: token,
  ...
  async login(dto: LoginDto) {
    ...
    const token = this.jwtService.sign({ userId: user.id, studioId: user.studioId });
    return {
      accessToken: token,
      user: { id: user.id, email: user.email, firstName: user.firstName, lastName: user.lastName },
    };
  ...
  async getProfile(userId: string) {
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      select: { id: true, email: true, firstName: true, lastName: true, role: true, studioId: true, specialty: true },
    });
