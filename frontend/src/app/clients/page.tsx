...
export default function ClientsPage() {
  ...
  const filtered = clients.filter((c) =>
    `${c.artistName} ${c.contactName} ${c.phone} ${c.email || ''} ${c.city || ''} ${c.notes || ''}`.toLowerCase().includes(search.toLowerCase())
  );
      ...
      {showForm && (
        <div className="card mb-6">
          ...
          <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input type="text" placeholder="Sanatçı adı *" value={form.artistName} onChange={(e) => setForm({ ...form, artistName: e.target.value })} className="input-field" required />
            ...
            <input type="text" placeholder="Notlar (örn: orta kayıt tercihi)" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="input-field" />
              ...
              <tbody>
                {filtered.map((c) => (
                  <tr key={c.id} className="border-b last:border-0" style={{ borderColor: 'var(--border-color)' }}>
                    <td className="p-4 font-medium" style={{ color: 'var(--text-primary)' }}>{c.artistName}</td>
