...
import { LayoutDashboard, Users, Mic2, Headphones, Package, Calendar, Settings, Moon, Sun, LogOut } from 'lucide-react';

const navItems = [
  { href: '/dashboard', label: 'Panel', icon: LayoutDashboard },
  { href: '/clients', label: 'Sanatçılar', icon: Users },
  { href: '/studio-rooms', label: 'Odalar', icon: Mic2 },
  { href: '/equipment', label: 'Ekipman', icon: Headphones },
  { href: '/deliveries', label: 'Teslimat', icon: Package },
  { href: '/sessions', label: 'Seanslar', icon: Calendar },
      ...
      >
        <div className="p-6 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-display text-lg font-bold" style={{ color: 'var(--text-nav)' }}>PitchPulse</div>
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  color: active ? 'var(--neon-cyan)' : 'var(--text-nav-muted)',
                }}
              >
                <Icon size={18} />
                {item.label}
      ...
      <main className="flex-1 md:ml-64">
        ...
        >
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center font-display font-bold text-sm" style={{ background: 'linear-gradient(135deg, var(--neon-purple), var(--neon-cyan))', color: 'white' }}>P</div>
            <span className="font-display font-bold text-sm" style={{ color: 'var(--text-nav)' }}>PitchPulse</span>
          </div>
      ...
      >
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon size={20} style={{ color: active ? 'var(--neon-cyan)' : 'var(--text-nav-muted)' }} />
              <span className="text-[10px] font-medium" style={{ color: active ? 'var(--neon-cyan)' : 'var(--text-nav-muted)' }}>
                {item.label}
              </span>
            </Link>
          );
